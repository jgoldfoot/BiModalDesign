...
1. **Whitepaper Updates:** Add a section to `docs/whitepaper.md` discussing
   Operator, Project Mariner, and UFO², including their benchmark scores on
   WebArena, OSWorld, ScreenSpot, and WebVoyager.
2. **New Implementation Pattern:** Document a pattern for "Demonstration
   Learning Optimization" focusing on stable identifiers and explicit
   boundaries.
3. **New Example:** Add `examples/operator-teach-and-repeat.html` to demonstrate
   this pattern.
4. **Update Citations:** Ensure the relevant projects and benchmarks are cited
